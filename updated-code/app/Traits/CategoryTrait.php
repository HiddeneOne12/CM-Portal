...
trait CategoryTrait
{
    private function sanitizeStoreCategoryData(array &$inputs): ?string
    {
        $validation = Validator::make($inputs, [
            ...
            'route'              => ['bail', 'required', 'string', 'max:155', new NoHtmlTags()],
        ...
        $inputs['module_category_id'] = sanitizeInput($inputs['module_category_id'] ?? 0, 'int');
        $inputs['module_name'] = sanitizeInput($inputs['module_name'] ?? '', 'alphanumeric');
        $inputs['route'] = sanitizeInput($inputs['route'] ?? '');
        $inputs['show_in_menu'] = sanitizeInput($inputs['show_in_menu'] ?? 0, 'int');
        $inputs['css_class'] = sanitizeInput($inputs['css_class'] ?? '', 'string');
