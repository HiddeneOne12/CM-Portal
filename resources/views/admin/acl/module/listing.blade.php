...
<div class="d-flex align-items-center gap-3 ms-auto">
    ...
    <div class="d-flex align-items-center position-relative">
        ...
        <input type="text" data-kt-vendor-table-filter="search" class="form-control form-control-solid w-100 min-w-200px ps-13" placeholder="Search Modules">
    </div>
</div>
@endif
@endpush
        ...
        <div class="card-header pe-5" id="kt_drawer_chat_messenger_header">
            <div class="card-title">
                <div class="d-flex justify-content-center flex-column me-3">
                    <span class="fs-4 fw-bold drawer-title text-gray-900 me-1 mb-2 lh-1">Module</span>
                </div>
            </div>
...
Swal.fire({
    ...
    if (result.value) {
        ...
        $.ajax({
            url: admin_url + "/acl/module/delete/" + did,
            method: "POST",
            data: {
                _token: token
            },
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    $(document).on('click', '.btn-add', function(e) {
        e.preventDefault();
        e.stopPropagation();
        $.ajax($.extend({ url: baseUrl + '/acl/module/add', type: 'GET' }, ajaxOpts))
            .done(function(res) {
                var d = parseJsonRes(res);
                if (d && d.responseCode === 1 && d.html) showDrawer(d.html);
                else if (d && d.msg) showAdminAlert('error', d.msg);
                else if (res && (res + '').trim().indexOf('<') === 0) showAdminAlert('error', 'Server returned a page instead of data. Open the add URL in a new tab to check.');
    ...
    $(document).on('click', '.btn-edit', function(e) {
        ...
        if (!token) return;
        $.ajax($.extend({ url: baseUrl + '/acl/module/edit/' + token, type: 'GET' }, ajaxOpts))
            .done(function(res) {
                var d = parseJsonRes(res);
                if (d && d.responseCode === 1 && d.html) showDrawer(d.html);
                else if (d && d.msg) showAdminAlert('error', d.msg);
    ...
    $(document).on('submit', '#kt_drawer_chat .drawer-body form', function(e) {
        e.preventDefault();
        var form = $(this), moduleToken = form.find('input[name="module_token"]').val(), url = baseUrl + '/acl/module/add';
        if (moduleToken) url = baseUrl + '/acl/module/edit/' + moduleToken;
    ...
    $('input[data-kt-vendor-table-filter="search"]').on('input', function() {
        var q = $(this).val();
        $.get(baseUrl + '/acl/module/search', { word: q }, function(res) {
            try {
                var d = typeof res === 'string' ? JSON.parse(res) : res;
