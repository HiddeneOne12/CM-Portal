...
@push('title')
Edit Admin User - {{ config('global.SITE_NAME') }}
@endpush
@section('header')
                ...
                <div class="card card-flush shadow-sm">
                    <div class="card-header pt-6 pb-4 border-bottom">
                        <div class="card-title d-flex align-items-center gap-3">
                            ...
                            <div>
                                <h3 class="card-label fw-bold text-gray-900 mb-0">Edit Admin User</h3>
                                <span class="text-muted fs-7">Editing: <strong>{{ $admin->user_name }}</strong></span>
                            </div>
                        </div>
                    </div>
                    <div class="card-body pt-6">
                        ...
                        <form method="POST" action="{{ route('admin.acl.admin-users.edit', encryptIdForUrl($admin->getKey())) }}">
                            @csrf
                            ...
                            <div class="row mb-6">
                                <div class="col-6">
                                    <label class="form-label fw-semibold text-gray-700">New Password</label>
                                ...
                                <div class="col-6">
                                    <label class="form-label fw-semibold text-gray-700">Confirm New Password</label>
                            ...
                            <div class="mb-8">
                                ...
                                @php
                                    ...
                                    $currentRole   = count($selectedRoles) ? $selectedRoles[0] : null;
                                @endphp
                                @if($roles->isEmpty())
                                ...
                                <div class="d-flex flex-wrap gap-3">
                                    {{-- None --}}
                                    <label class="role-option d-flex align-items-center gap-2 border rounded px-4 py-3 {{ is_null($currentRole) ? 'active' : '' }}">
                                        <input type="radio" name="roles[]" value="" class="d-none role-radio"
                                               {{ is_null($currentRole) ? 'checked' : '' }}>
                                        <span class="role-indicator border border-2 rounded-circle d-flex align-items-center justify-content-center flex-shrink-0"
                                            ...
                                            <span class="role-dot rounded-circle" style="width:8px;height:8px;{{ is_null($currentRole) ? '' : 'display:none;' }}"></span>
                                        </span>
                                        <span class="fw-semibold text-gray-700 fs-7">None</span>
                                    </label>

                                    @foreach($roles as $role)
                                    @php $isSelected = $currentRole === (int)$role->ID; @endphp
                                    <label class="role-option d-flex align-items-center gap-2 border rounded px-4 py-3 {{ $isSelected ? 'active' : '' }}">
                                        <input type="radio" name="roles[]" value="{{ $role->ID }}" class="d-none role-radio"
                                               {{ $isSelected ? 'checked' : '' }}>
                                        <span class="role-indicator border border-2 rounded-circle d-flex align-items-center justify-content-center flex-shrink-0"
                                              style="width:18px;height:18px;">
                            ...
                            <div class="d-flex gap-3">
                                <button type="submit" class="btn btn-primary px-8">
                                    <i class="ki-duotone ki-check fs-4 me-1"></i> Update User
                                </button>
