        ...
        <div class="card mb-5 mb-xl-8">
            <div class="card-header border-0 pt-5">
                <div class="d-flex align-items-center position-relative my-1 me-3">
                    ...
                    <input type="text" name="search" value="{{ request('search') }}" id="reports-search-input" class="form-control form-control-solid w-250px ps-13" placeholder="Search Reports" autocomplete="off" />
                </div>
                <div class="card-toolbar">
                    @if (validatePermissions('admin/acl/reports/add'))
                    <a href="javascript:void(0)" class="btn btn-sm btn-primary btn-add-report"><i class="ki-duotone ki-plus fs-2"></i>Add Report</a>
                    @endif
                </div>
            </div>
            <div class="card-body py-3">
                <div class="table-responsive">
                    <table id="reports-table" class="table table-hover table-row-bordered table-row-gray-100 align-middle gs-0 gy-3">
                        <thead>
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>
                                    @if (validatePermissions('admin/acl/reports/edit'))
                                    <a href="javascript:void(0)" class="status-toggle text-decoration-none" data-id="{{ $row->id }}" data-url="{{ route('admin.acl.reports.toggle-status', encryptIdForUrl($row->id)) }}" data-status="{{ (int) $row->status }}" title="Click to toggle">
                                        @if((int) $row->status === 1)
                                ...
                                <td class="text-end">
                                    @if (validatePermissions('admin/acl/reports/edit'))
                                    <a href="javascript:void(0)" role="button" data-id="{{ $row->id }}" data-token="{{ encryptIdForUrl($row->id) }}" class="btn btn-light btn-sm btn-edit-report me-1">Edit</a>
                                    @endif
                                    @if (validatePermissions('admin/acl/reports/delete'))
                                    <a href="javascript:void(0)" data-id="{{ encryptIdForUrl($row->getKey()) }}" class="btn btn-sm btn-danger btn-delete" data-confirm-delete="Delete this report?">Delete</a>
                                    @endif
                                </td>
                            </tr>
                            @empty
                            <tr>
                                <td colspan="9" class="text-center text-muted py-10">No reports yet. Click "Add Report" to add one.</td>
                            </tr>
                            @endforelse
                        </tbody>
                    </table>
...
<div id="kt_activities" class="bg-body" data-kt-drawer-permanent="true" data-kt-drawer="true" data-kt-drawer-name="activities" data-kt-drawer-activate="true" data-kt-drawer-overlay="true" data-kt-drawer-width="{default:'300px', 'lg': '900px'}" data-kt-drawer-direction="end" data-kt-drawer-toggle="#kt_activities_toggle" data-kt-drawer-close="#kt_activities_close">
    <div class="card shadow-none border-0 rounded-0 w-100">
        <div class="card-header" id="kt_activities_header">
            <h3 class="card-title drawer-title fw-bold text-gray-900">Reports</h3>
            <div class="card-toolbar">
...
Swal.fire({
    ...
    if (result.value) {
        ...
        $.ajax({
            url: admin_url + "/acl/reports/delete/" + did,
            method: "POST",
            data: {
                _token: token
            },
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    $(document).on('click', '.btn-add-report', function(e) {
        e.preventDefault();
        $.ajax($.extend({ url: baseUrl + '/acl/reports/add', type: 'GET' }, ajaxOpts))
            .done(function(res) {
                var d = parseJsonRes(res);
                if (d && d.responseCode === 1 && d.html) showDrawer(d.html, 'Add Report');
    ...
    $(document).on('click', '.btn-edit-report', function(e) {
        ...
        if (!token) return;
        $.ajax($.extend({ url: baseUrl + '/acl/reports/edit/' + token, type: 'GET' }, ajaxOpts))
            .done(function(res) {
                var d = parseJsonRes(res);
                if (d && d.responseCode === 1 && d.html) showDrawer(d.html, 'Edit Report');
                else if (d && d.msg) showAdminAlert('error', d.msg);
                else showAdminAlert('error', 'Could not load form');
            })
        ...
        if (searchEl) {
            searchEl.addEventListener('input', function() {
                ...
                searchTimer = setTimeout(function() {
                    var url = '{{ route("admin.acl.reports.listing") }}';
    ...
    $(document).on('submit', '#kt_activities .drawer-body form', function(e) {
        e.preventDefault();
        var form = $(this)[0];
        var reportToken = $(this).find('input[name="report_token"]').val();
        var url = baseUrl + '/acl/reports/add';
        if (reportToken) url = baseUrl + '/acl/reports/edit/' + reportToken;
