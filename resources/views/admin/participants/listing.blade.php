        ...
        <div class="card mb-5 mb-xl-8">
            <div class="card-header border-0 pt-5">
                <div class="d-flex align-items-center position-relative my-1 me-3">
                    ...
                    <input type="text" name="search" value="{{ request('search') }}" id="participants-search-input" class="form-control form-control-solid w-250px ps-13" placeholder="Search Participants" autocomplete="off" />
                </div>
                <div class="card-toolbar">
                    @if (validatePermissions('admin/acl/participants/add'))
                    <a href="javascript:void(0)" class="btn btn-sm btn-primary btn-add-participant"><i class="ki-duotone ki-plus fs-2"></i>Add Participant</a>
                    @endif
                </div>
            </div>
            <div class="card-body py-3">
                <div class="table-responsive">
                    <table id="participants-table" class="table table-hover table-row-bordered table-row-gray-100 align-middle gs-0 gy-3">
                        <thead>
                        ...
                        <tbody>
                            ...
                            <tr>
                                <td>
                                    @if($row->image)
                                    <img src="{{ asset($row->image) }}" alt="" class="rounded" style="width: 50px; height: 50px; object-fit: cover;">
                                    @else
                                    <span class="text-muted">—</span>
                                    @endif
                                </td>
                                <td>
                                    @if (validatePermissions('admin/acl/participants/edit'))
                                    <a href="javascript:void(0)" class="status-toggle text-decoration-none" data-id="{{ $row->id }}" data-url="{{ route('admin.acl.participants.toggle-status', encryptIdForUrl($row->id)) }}" data-status="{{ (int) $row->status }}" title="Click to toggle">
                                        @if((int) $row->status === 1)
                                ...
                                <td>{{ $row->created_at ? $row->created_at->format('M j, Y g:i A') : '—' }}</td>
                                <td>{{ $row->updated_at ? $row->updated_at->format('M j, Y g:i A') : '—' }}</td>
                                <td class="text-end">
                                    @if (validatePermissions('admin/acl/participants/edit'))
                                    <a href="javascript:void(0)" role="button" data-id="{{ $row->id }}" data-token="{{ encryptIdForUrl($row->id) }}" class="btn btn-light btn-sm btn-edit-participant me-1">Edit</a>
                                    @endif
                                    @if (validatePermissions('admin/acl/participants/delete'))
                                    <a href="javascript:void(0)" data-id="{{ encryptIdForUrl($row->getKey()) }}" class="btn btn-sm btn-danger btn-delete" data-confirm-delete="Delete this participant?">Delete</a>
                                    @endif
                                </td>
                            </tr>
                            @empty
                            <tr>
                                <td colspan="9" class="text-center text-muted py-10">No participants yet. Click "Add Participant" to add one.</td>
                            </tr>
                            @endforelse
                        </tbody>
                    </table>
...
<div id="kt_activities" class="bg-body" data-kt-drawer-permanent="true" data-kt-drawer="true" data-kt-drawer-name="activities" data-kt-drawer-activate="true" data-kt-drawer-overlay="true" data-kt-drawer-width="{default:'300px', 'lg': '900px'}" data-kt-drawer-direction="end" data-kt-drawer-toggle="#kt_activities_toggle" data-kt-drawer-close="#kt_activities_close">
    <div class="card shadow-none border-0 rounded-0 w-100">
        <div class="card-header" id="kt_activities_header">
            <h3 class="card-title drawer-title fw-bold text-gray-900">Participants</h3>
            <div class="card-toolbar">
...
Swal.fire({
    ...
    if (result.value) {
        ...
        $.ajax({
            url: admin_url + "/acl/participants/delete/" + did,
            method: "POST",
            data: {
                _token: token
            },
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    $(document).on('click', '.btn-add-participant', function(e) {
        e.preventDefault();
        $.ajax($.extend({ url: baseUrl + '/acl/participants/add', type: 'GET' }, ajaxOpts))
            .done(function(res) {
                var d = parseJsonRes(res);
                if (d && d.responseCode === 1 && d.html) showDrawer(d.html, 'Add Participant');
    ...
    $(document).on('click', '.btn-edit-participant', function(e) {
        e.preventDefault();
        var token = $(this).data('token');
        if (!token) return;
        $.ajax($.extend({ url: baseUrl + '/acl/participants/edit/' + token, type: 'GET' }, ajaxOpts))
            .done(function(res) {
                var d = parseJsonRes(res);
                if (d && d.responseCode === 1 && d.html) showDrawer(d.html, 'Edit Participant');
                else if (d && d.msg) showAdminAlert('error', d.msg);
                else showAdminAlert('error', 'Could not load form');
            })
    ...
    (function() {
        var searchEl = document.getElementById('participants-search-input');
        var searchTimer;
        if (searchEl) {
            searchEl.addEventListener('input', function() {
                ...
                searchTimer = setTimeout(function() {
                    var url = '{{ route("admin.acl.participants.listing") }}';
    ...
    $(document).on('submit', '#kt_activities .drawer-body form', function(e) {
        e.preventDefault();
        var form = $(this)[0];
        var participantToken = $(this).find('input[name="participant_token"]').val();
        var url = baseUrl + '/acl/participants/add';
        if (participantToken) url = baseUrl + '/acl/participants/edit/' + participantToken;
