<form method="post" action="{{ url('/cmcontrol/acl/training/edit/' . $row->id) }}" enctype="multipart/form-data">
    @csrf
    <input type="hidden" name="eid" value="{{ $row->id }}">
    <input type="hidden" name="training_token" value="{{ encryptIdForUrl($row->id) }}">
    ...
    <div class="fv-row mb-5">
        ...
        <input type="text" name="title" class="form-control form-control-solid" placeholder="Training title" value="{{ old('title', $row->title) }}" required />
    </div>
    <div class="fv-row mb-5">
        <label class="fs-6 fw-semibold mb-2">Description</label>
        <textarea name="description" class="form-control form-control-solid" rows="4" placeholder="Description shown on the training video page">{{ old('description', $row->description) }}</textarea>
    </div>
    <div class="fv-row mb-5">
        ...
        @else
        <span class="text-muted fs-7">One of Video or Training image is required.</span>
        @endif
    ...
    <div class="fv-row mb-5">
        ...
        <div class="mb-2"><img src="{{ asset($row->training_image) }}" alt="" class="rounded" style="max-height: 80px;"></div>
        <span class="text-muted fs-7">Leave empty to keep current. Alternative to video.</span>
        @else
        <span class="text-muted fs-7">Alternative to video. One of Video or Training image is required.</span>
        @endif
