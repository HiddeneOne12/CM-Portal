        ...
        <div class="card mb-5 mb-xl-8">
            <div class="card-header border-0 pt-5">
                <div class="d-flex align-items-center position-relative my-1 me-3">
                    ...
                    <input type="text" name="search" value="{{ request('search') }}" id="hero-search-input" class="form-control form-control-solid w-250px ps-13" placeholder="Search Hero" autocomplete="off" />
                </div>
                <div class="card-toolbar">
                    @if (validatePermissions('admin/acl/hero/add'))
                    <a href="javascript:void(0)" class="btn btn-sm btn-primary btn-add-hero"><i class="ki-duotone ki-plus fs-2"></i>Add Hero</a>
                    @endif
                </div>
            </div>
            <div class="card-body py-3">
                <div class="table-responsive">
                    <table id="hero-table" class="table table-hover table-row-bordered table-row-gray-100 align-middle gs-0 gy-3">
                        <thead>
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>
                                    @if (validatePermissions('admin/acl/hero/edit'))
                                    <a href="javascript:void(0)" class="status-toggle text-decoration-none" data-id="{{ $row->id }}" data-url="{{ route('admin.acl.hero.toggle-status', encryptIdForUrl($row->id)) }}" data-status="{{ (int) $row->status }}" title="Click to toggle">
                                        @if((int) $row->status === 1)
                                ...
                                <td class="text-end">
                                    @if (validatePermissions('admin/acl/hero/edit'))
                                    <a href="javascript:void(0)" role="button" data-id="{{ $row->id }}" data-token="{{ encryptIdForUrl($row->id) }}" class="btn btn-light btn-sm btn-edit-hero me-1">Edit</a>
                                    @endif
                                    @if (validatePermissions('admin/acl/hero/delete'))
                                    <a href="javascript:void(0)" data-id="{{ encryptIdForUrl($row->getKey()) }}" class="btn btn-sm btn-danger btn-delete"  data-confirm-delete="Delete this hero item?">Delete</a>
                                    @endif
                                </td>
                            </tr>
                            @empty
                            <tr>
                                <td colspan="8" class="text-center text-muted py-10">No hero items yet. Click "Add Hero" to add one.</td>
                            </tr>
                            @endforelse
                        </tbody>
                    </table>
                </div>
                @if(isset($heroItems) && $heroItems->hasPages())
                <div class="d-flex justify-content-between align-items-center flex-wrap pt-5">
                    <div class="fs-7 text-gray-700">{{ $heroItems->firstItem() ?? 0 }} - {{ $heroItems->lastItem() ?? 0 }} of {{ $heroItems->total() }}</div>
                    <div>{{ $heroItems->withQueryString()->links() }}</div>
                </div>
                @endif
            </div>
        </div>
...
<div id="kt_activities" class="bg-body" data-kt-drawer-permanent="true" data-kt-drawer="true" data-kt-drawer-name="activities" data-kt-drawer-activate="true" data-kt-drawer-overlay="true" data-kt-drawer-width="{default:'300px', 'lg': '900px'}" data-kt-drawer-direction="end" data-kt-drawer-toggle="#kt_activities_toggle" data-kt-drawer-close="#kt_activities_close">
    <div class="card shadow-none border-0 rounded-0 w-100">
        <div class="card-header" id="kt_activities_header">
            <h3 class="card-title drawer-title fw-bold text-gray-900">Hero</h3>
            <div class="card-toolbar">
...
Swal.fire({
    ...
    if (result.value) {
        ...
        $.ajax({
            url: admin_url + "/acl/hero/delete/" + did,
            method: "POST",
            data: {
                _token: token
            },
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    $(document).on('click', '.btn-add-hero', function(e) {
        e.preventDefault();
        $.ajax($.extend({ url: baseUrl + '/acl/hero/add', type: 'GET' }, ajaxOpts))
            .done(function(res) {
                var d = parseJsonRes(res);
                if (d && d.responseCode === 1 && d.html) showDrawer(d.html, 'Add Hero');
    ...
    $(document).on('click', '.btn-edit-hero', function(e) {
        e.preventDefault();
        var token = $(this).data('token');
        if (!token) return;
        $.ajax($.extend({ url: baseUrl + '/acl/hero/edit/' + token, type: 'GET' }, ajaxOpts))
            .done(function(res) {
                var d = parseJsonRes(res);
                if (d && d.responseCode === 1 && d.html) showDrawer(d.html, 'Edit Hero');
                else if (d && d.msg) showAdminAlert('error', d.msg);
                else showAdminAlert('error', 'Could not load form');
            })
        ...
        if (heroSearch) {
            heroSearch.addEventListener('input', function() {
                ...
                heroSearchTimer = setTimeout(function() {
                    var url = '{{ route("admin.acl.hero.listing") }}';
    ...
    $(document).on('click', '.video-preview', function(e) {
        ...
        var html = '' +
            '<div class="p-5">' +
                ...
                '</div>' +
            '</div>';
        showDrawer(html, 'Preview Hero Video');
    });
    ...
    $(document).on('submit', '#kt_activities .drawer-body form', function(e) {
        e.preventDefault();
        var form = $(this)[0];
        var heroToken = $(this).find('input[name="hero_token"]').val();
        var url = baseUrl + '/acl/hero/add';
        if (heroToken) url = baseUrl + '/acl/hero/edit/' + heroToken;
            ...
            .fail(function(xhr) {
                ...
                if (xhr.status === 422 && xhr.responseJSON) {
                    ...
                    if (j.message) msg = j.message;
                        ...
                        var first = [];
                        Object.keys(j.errors).forEach(function(k) { (j.errors[k] || []).forEach(function(m) { first.push(m); }); });
                        if (first.length) msg = first.join(' ');
                    }
                } else {
                    var d = xhr.responseText ? parseJsonRes(xhr.responseText) : null;
                    if (d && d.msg) msg = d.msg;
                }
